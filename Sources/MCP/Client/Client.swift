...
public actor Client {
    ...
    public struct Configuration: Hashable, Codable, Sendable {
        ...
        ///
        /// A peer that never answers `server/discover` is treated as initialization-era, because a
        /// pre-2026 server may ignore an unrecognized method rather than reporting one. The budget
        /// must therefore cover the peer's cold start, not just its steady-state response time: a
        /// server still loading an interpreter looks exactly like one that will never answer, and
        /// too short a budget silently downgrades a modern peer that is merely slow to start.
